...
def addInterviewExperience(request):
    ...
    if request.method == "POST":
        try:
            form = addInterviewExperienceForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
        ...
        form = addInterviewExperienceForm()
    context = {"form": form}
...
def jobList(request):
    ...
    context = {"jobs": jobs}
    return render(request, "home/jobList.html", context=context)
...
def addStudentJob(request):
    if request.method == "POST":
        ...
        try:
            ...
            studentJob = StudentJob.objects.create(
                job=job, student=student, round=round
            )
        ...
        try:
            ...
            if stu.isPR == False:
                return HttpResponse("403 - Unauthorized")
            studentJob = StudentJob.objects.filter(
                job=job, student=student, round=round
